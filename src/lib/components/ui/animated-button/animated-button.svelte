...
<script lang="ts">
	...
	const defaultSpringTransition = { type: 'spring' as const, stiffness: 400, damping: 25 };
	const motionTransition = $derived(transitionProp ?? defaultSpringTransition);
...
{#if href}
	{#if animated}
		<MotionA
			...
			whileTap={whileTap ?? getTapScale()}
			whileHover={whileHover ?? getHoverEffect()}
			transition={motionTransition}
			{...restProps}
		>
			{@render children?.()}
